...
def initialize():
    ...
    PATH_TO_FROZEN_GRAPH = 'output_FRCNN/frozen_inference_graph.pb'

    # List of the strings that is used to add correct label for each box.
    PATH_TO_LABELS = 'training_sat_image/data/detection.pbtxt'
    ...
    TEST_IMAGES_DIR = 'test_images_overlapped/test_images'
...
def start_detecting():
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            new_global_boxes = [list() for f in range(len(outpath))]
            new_global_boxes_sup = list()

            for i in range(len(outpath)):
                image = Image.open(os.path.join(TEST_IMAGES_DIR, outpath[i]))
...
def non_max_suppression_fast(boxes, overlapThresh):
    ...
    np.savetxt("training_sat_image/all_boxes.csv", all_boxes, delimiter=",")
    np.savetxt("training_sat_image/all_boxes_dec.csv", all_boxes, fmt="%d", delimiter=",")
    single_boxes = non_max_suppression_fast(all_boxes, 0.1)
    print(single_boxes[1803])
    np.savetxt("training_sat_image/single_boxes.csv", all_boxes, fmt="%d", delimiter=",")
